...
CREATE TABLE idempotency_keys (
    id VARCHAR(255) PRIMARY KEY,           -- Client-provided idempotency key
    user_id UUID NOT NULL,                 -- Owner of the request
    request_hash VARCHAR(64),              -- SHA-256 hash of request payload
    response_status INT,                   -- HTTP status code (200, 201, etc.)
    response_body JSONB,                   -- Full response payload
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    expires_at TIMESTAMPTZ NOT NULL        -- For cleanup (24 hours from creation)
);
...
sequenceDiagram
    ...
    Client->>API: POST /bookings<br/>Idempotency-Key: abc-123
    
    API->>Service: createBooking(request, key)
    
    Note over Service: BEGIN TRANSACTION
    
    Service->>DB: INSERT idempotency_keys<br/>(id='abc-123', user_id=...)
    DB-->>Service: Success (row locked)
    
    Service->>Service: Execute business logic
    Service->>DB: INSERT bookings (...)
    Service->>DB: UPDATE seats (...)
    
    Service->>DB: UPDATE idempotency_keys<br/>SET response_status=201, response_body={...}
...
### 2. Retry Request (Duplicate Detection)
...
sequenceDiagram
    ...
    Client->>API: POST /bookings<br/>Idempotency-Key: abc-123<br/>(RETRY)
    ...
    Service->>DB: INSERT idempotency_keys<br/>(id='abc-123', user_id=...)
    DB-->>Service: ❌ Duplicate key violation
    
    Service->>DB: SELECT * FROM idempotency_keys<br/>WHERE id='abc-123'
    DB-->>Service: { response_status: 201,<br/>response_body: {...} }
    
    Note over Service: ROLLBACK (no changes made)
    
    Service-->>API: Cached BookingResponse
    API-->>Client: 201 Created<br/>(same response as original)
```

### 3. Conflict Detection (Payload Mismatch)
...
sequenceDiagram
    ...
    Note over Client: Original: { seatIds: [1, 2] }
    Note over Client: Retry: { seatIds: [1, 3] } ⚠️
    
    Client->>API: POST /bookings<br/>Idempotency-Key: abc-123<br/>Body: { seatIds: [1, 3] }
    
    API->>Service: createBooking(request, key)
    
    Service->>DB: SELECT * FROM idempotency_keys<br/>WHERE id='abc-123'
    DB-->>Service: { request_hash: 'hash-of-[1,2]' }
    
    Service->>Service: Compute hash of [1, 3]
    Service->>Service: Compare hashes
    Note over Service: ❌ MISMATCH
    
    Service-->>API: 409 Conflict
    API-->>Client: 409 Conflict<br/>error: "IDEMPOTENCY_CONFLICT"
```

### 4. Failure & Retry (Rollback Path)
...
sequenceDiagram
    ...
    Service->>DB: INSERT idempotency_keys (id='abc-123')
    DB-->>Service: Success
    ...
    Note over DB: Idempotency key rolled back
    ...
    Client->>API: POST /bookings<br/>Idempotency-Key: abc-123<br/>(RETRY after failure)
    ...
    Service->>DB: INSERT idempotency_keys (id='abc-123')
    Note over DB: ✅ No conflict (previous rolled back)
    DB-->>Service: Success
...
- `request_hash`: 64 bytes (SHA-256 hex)
- `response_body`: ~500 bytes (average booking response)
- `timestamps`: 24 bytes (3 × 8 bytes)
- **Total:** ~640 bytes per key

**Daily Volume (Estimated):**
- 10,000 bookings/day
- 20% retry rate = 2,000 duplicate requests
- Total keys stored: 12,000/day
- Storage: 12,000 × 640 bytes = ~7.7 MB/day
- After 24h expiry: ~7.7 MB steady state

**Annual Storage:** 7.7 MB (constant, not cumulative due to expiry)
...
-- Time: ~2ms (single row, primary key index)
```
...
-- Time: ~50ms for 10,000 rows (indexed scan)
```

**Total Overhead per Request:**
- First request: +2ms (INSERT)
- Retry request: +1ms (SELECT)
...
### Why Single Generic Table?
**Answer:** Avoid schema duplication. Single table with JSONB supports all entity types without N tables.

### Why PostgreSQL Over Redis?
**Answer:** ACID guarantees with business logic. Row-level locking eliminates distributed consensus complexity.

### Why Store Full Response?
**Answer:** Client gets exact same response on retry, including computed fields (timestamps, IDs, prices).

### Why 24-Hour Expiry?
**Answer:** Balance retry window (covers network outages) vs storage cost (7.7 MB steady state).

### Why Include Request Hash?
**Answer:** Detect payload changes between retries. Prevents malicious or accidental data substitution (409 Conflict).
